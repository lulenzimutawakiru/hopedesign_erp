...
/**
 * KCB bank integration desk.
 *
 * Two halves of one integration meet on this screen. The inbound half is the
 * bank pushing payment notifications at /api/integrations/kcb, which the API
 * verifies with a stored KCB public key and posts into the bank ledger. This
 ...
 *
 * Outbound funds transfer is deliberately absent. It needs KCB onboarding
...
/** Mirrors services/kcb/config.ts. Never carries secret material. */
type KcbConfigView = {
  configured: boolean;
...
type KcbNotificationPage = { rows: KcbNotification[]; total: number; limit: number; offset: number };

type KcbConnectionCheck = { key: string; label: string; ok: boolean; detail: string; critical: boolean };

type KcbConnectionTestResult = {
  ok: boolean;
  status: string;
  testedAt: string;
  checks: KcbConnectionCheck[];
  config: KcbConfigView;
};

type KcbInvoiceCandidate = {
  id: number;
  invoiceNo: string;
  customerName: string | null;
  currency: string | null;
...
const KCB_TABS: [string, string][] = [
  ['overview', 'Overview'],
  ['notifications', 'Notifications'],
  ['setup', 'Setup'],
];

const KCB_STATUSES = ['RECEIVED', 'POSTED', 'MATCHED', 'REJECTED'];
const KCB_TYPES = ['ACCOUNT', 'TILL', 'VALIDATION'];
const KCB_PAGE_SIZES = [25, 50, 100];

/** What each stored status means for the money, in plain words. */
const KCB_STATUS_MEANING: Record<string, string> = {
...
const KCB_REJECT_MEANING: Record<string, string> = {
  UNMAPPED_ACCOUNT: 'Nothing matched the account or till it was paid into, so it was not posted.',
  SIGNATURE_INVALID: 'The Signature header did not verify against a stored KCB public key.',
  DUPLICATE: 'This transaction id or request id had already been received.',
};

/** Where the bank posts to. Shown so the URL can be handed to KCB exactly. */
const KCB_INBOUND_PATHS: [string, string][] = [
  ['/api/integrations/kcb/account-notification', 'Account notification'],
  ['/api/integrations/kcb/till-notification', 'Till notification'],
  ['/api/integrations/kcb/validation', 'Bill validation (authorisation only, never posted)'],
];

/** Currency-prefixed money. A null amount is an em dash, never a zero. */
const kcbMoney = (amount: number | null | undefined, currency: string | null | undefined): string =>
  amount === null || amount === undefined
    ? '\u2014'
    : `${currency ? `${currency} ` : ''}${fmtMoney(amount)}`;

function kcbDay(v: unknown): string {
  if (!v) return '\u2014';
...
function kcbWhen(v: unknown): string {
  if (!v) return '\u2014';
  const d = new Date(String(v));
  if (Number.isNaN(d.getTime())) return String(v);
...
function kcbAgo(v: unknown): string {
  if (!v) return '\u2014';
  const d = new Date(String(v));
  if (Number.isNaN(d.getTime())) return String(v);
...
const kcbRef = (n: KcbNotification): string =>
  n.transactionReference ?? n.kcbTransactionId ?? n.requestId ?? String(n.id);
 ...
 */
function KcbStatusChip({ status }: { status: string }) {
  ...
  return (
    <span className={`badge ${meta.tone}`} title={KCB_STATUS_MEANING[s] ?? undefined}>
...
function KcbCheckChip({ ok }: { ok: boolean }) {
  return (
...
function KcbSecretState({ label, present, note }: { label: string; present: boolean; note?: string }) {
  return (
    <div className="field">
      <label>{label}</label>
      <div>
...
function KcbSelect({ value, onChange, options, label, placeholder, labels }: {
  value: string;
 ...
 * actually accept.
 */
export function FinanceKcb() {
  const { user } = useAuth();
  const qs = useHashQuery();

  const tabParam = qs.get('tab') ?? '';
  const tab = KCB_TABS.some(([k]) => k === tabParam) ? tabParam : 'overview';
  ...
  const pageSize = KCB_PAGE_SIZES.includes(sizeParam) ? sizeParam : 50;

  const canManage = can(user, 'finance.kcb.manage');
  const canTest = can(user, 'finance.kcb.test');
  ...
  const [status, setStatus] = useState<KcbStatusView | null>(null);
  const [notifications, setNotifications] = useState<KcbNotificationPage | null>(null);
  const [accounts, setAccounts] = useState<KcbBankAccount[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState('');
  const [notice, setNotice] = useState('');
  const [loadFailures, setLoadFailures] = useState<Array<{ panel: string; message: string }>>([]);
  const [testResult, setTestResult] = useState<KcbConnectionTestResult | null>(null);
  const [testing, setTesting] = useState(false);
  const [busy, setBusy] = useState(false);
  const [matchTarget, setMatchTarget] = useState<KcbNotification | null>(null);
  const [unmatchTarget, setUnmatchTarget] = useState<KcbNotification | null>(null);
  ...
  const writeQuery = useCallback(
    (patch: Record<string, string | number | undefined>) => {
      ...
      navigate('/finance/kcb', { query: next });
    },
    [qs]
  );
  ...
  const load = useCallback(
    async (opts: { silent?: boolean } = {}) => {
      ...
      const jobs: Array<[string, Promise<unknown>]> = [
        [
          'integration status',
          api<{ data: KcbStatusView }>('/api/ops/finance/kcb/status')
            .then((r) => setStatus(r.data ?? null))
            .catch((e) => { failures.push({ panel: 'integration status', message: reason(e) }); }),
        ],
        [
          'payment notifications',
          api<{ data: KcbNotificationPage }>(`/api/ops/finance/kcb/notifications?${params.toString()}`)
      ...
      if (canSeeBanks) {
        jobs.push([
          'bank accounts',
          api<{ data: { rows: KcbBankAccount[] } }>('/api/ops/finance/banks')
  ...
  const runTest = async () => {
    ...
    setError('');
    try {
      const r = await api<{ data: KcbConnectionTestResult }>('/api/ops/finance/kcb/test-connection', { method: 'POST' });
  ...
  const saveConfig = (payload: Rec) =>
    commit('Integration settings saved.', () =>
      api('/api/ops/finance/kcb/config', { method: 'PATCH', body: JSON.stringify(payload) })
    );
  ...
  const matchInvoice = async (invoiceId: number) => {
    ...
    const ok = await commit('Payment reconciled to the invoice.', async () => {
      const r = await api<{ data: KcbMatchResult }>(
        `/api/ops/finance/kcb/notifications/${target?.id}/match`,
    ...
    if (ok && gap !== null && gap !== 0) {
      setNotice(
        gap > 0
          ? `Payment reconciled. It paid more than the invoice owed by ${kcbMoney(gap, gapCurrency)}.`
          : `Payment reconciled. It was short of the invoice by ${kcbMoney(Math.abs(gap), gapCurrency)}.`
      );
    }
    return ok;
  };

  const unmatch = (n: KcbNotification) =>
    commit('Reconciliation removed. The payment line itself is untouched.', () =>
      api(`/api/ops/finance/kcb/notifications/${n.id}/unmatch`, { method: 'POST' })
    );
    ...
    <div className="page">
      <header className="page-head">
        <div>
          ...
          <h1>KCB bank integration</h1>
          <p className="muted">
            Payment notifications pushed by KCB, and the customer invoices the money settled.
          </p>
        </div>
      ...
      <nav className="toolbar" aria-label="KCB sections" style={{ gap: 6 }}>
        {KCB_TABS.map(([key, label]) => (
          <button
            key={key}
          ...
          <>
            <section className="card card-pad" style={{ marginBottom: 14 }}>
              ...
              <p className="muted" style={{ marginTop: 4 }}>
                {ready
                  ? 'A notification signed by KCB will verify and be posted into the settlement account below.'
                  : 'KCB notifications will be refused or held until every item below is resolved.'}
              </p>
              {warnings.length > 0 ? (
              ...
              <div className="kpi-card">
                ...
                <span className="kpi-sub">
                  {kcbMoney(totals?.unreconciledAmount, config?.currency)} in the bank ledger with no invoice
                </span>
              </div>
              <div className="kpi-card">
              ...
              <div className="kpi-card">
                ...
                <span className="kpi-value">{kcbAgo(status?.lastNotifiedAt)}</span>
                <span className="kpi-sub">{kcbWhen(status?.lastNotifiedAt)}</span>
              </div>
            </div>
            ...
            <section className="card card-pad" style={{ marginBottom: 14 }}>
              ...
              {config === null ? (
                <EmptyState
                  title="No KCB integration yet"
                  body="Creating one stores the KCB public key this server verifies notifications with. Use Configure to set it up."
                />
              ) : (
                  ...
                  <table className="data">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          {config.publicKeyFingerprint ? (
                            ...
                            <span>From the KCB_PUBLIC_KEY environment variable</span>
                          ) : (
                      ...
                      <tr>
                        ...
                        <td>{config.lastTestedAt ? kcbWhen(config.lastTestedAt) : 'Never'}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            {testResult && (
              <section className="card card-pad" style={{ marginBottom: 14 }}>
                <div className="card-head">
                  ...
                  <span className="muted" style={{ fontSize: 12 }}>Tested {kcbWhen(testResult.testedAt)}</span>
                </div>
                <div className="table-wrap">
                  <table className="data">
                    <thead>
                    ...
                    <tbody>
                      {testResult.checks.map((check) => (
                        <tr key={check.key} className={!check.ok && check.critical ? 'row-warn' : undefined}>
                          ...
                          <td><KcbCheckChip ok={check.ok} /></td>
                          <td>{check.detail}</td>
                        </tr>
                      ))}
                    </tbody>
        ...
        <section className={`card card-pad${refreshing && !loading ? ' is-refreshing' : ''}`} aria-busy={refreshing && !loading}>
          <div className="toolbar" style={{ marginBottom: 10 }}>
            <input
              ...
              type="search"
              value={searchDraft}
              aria-label="Search KCB payment notifications"
            ...
            />
            <KcbSelect value={statusFilter} onChange={(v) => writeQuery({ status: v, page: undefined })}
              options={KCB_STATUSES} label="Filter by status" placeholder="Any status" />
            <KcbSelect value={typeFilter} onChange={(v) => writeQuery({ type: v, page: undefined })}
              options={KCB_TYPES} label="Filter by notification type" placeholder="Any type"
              labels={{
                ACCOUNT: 'Account notification',
                TILL: 'Till notification',
                VALIDATION: 'Validation (authorisation only)',
              }} />
            <KcbSelect value={matchedFilter} onChange={(v) => writeQuery({ matched: v, page: undefined })}
          ...
          {loading ? <Skeleton rows={6} /> : rows.length === 0 ? (
            hasFilters ? (
              <EmptyState title="No notifications match these filters"
                body="Nothing matches the current filters. Clear them to see everything KCB has sent."
                action="Clear filters" onAction={() => { setSearchDraft(''); clearFilters(); }} />
            ) : (
              <EmptyState title="No KCB notifications yet"
                body="Point KCB at the inbound URLs on the Setup tab. Payments it sends will appear here, and money is only posted once a settlement account resolves." />
            )
          ) : (
                  ...
                  return (
                    <div key={`card-${n.id}`} className="record-card">
                      <div className="record-card-top">
                        <strong className="cell-mono">{kcbRef(n)}</strong>
                        <KcbStatusChip status={n.status} />
                      </div>
                      <div className="record-card-meta">
                        <span>{kcbWhen(n.transactionAt ?? n.createdAt)}</span>
                        <span>{isValidation ? 'Authorisation only' : n.notificationType}</span>
                        <span>{kcbMoney(n.amount, n.currency ?? config?.currency)}</span>
                      </div>
                      <div>{n.customerName ?? 'Customer not named'}</div>
                      {n.status === 'REJECTED' && n.rejectReason && (
                        <div className="muted" style={{ fontSize: 11.5 }}>
                          {KCB_REJECT_MEANING[n.rejectReason] ?? n.rejectReason}
                        </div>
                      )}
                    ...
                    return (
                      <tbody key={n.id}>
                        <tr className={n.status === 'RECEIVED' ? 'row-warn' : undefined}>
                          <td className="cell-mono">{kcbRef(n)}</td>
                          <td>{kcbWhen(n.transactionAt ?? n.createdAt)}</td>
                          <td>
                            {isValidation ? (
                              <>
                          ...
                          <td className="cell-num">{kcbMoney(n.amount, n.currency ?? config?.currency)}</td>
                          <td>
                            {n.bankAccountCode
                          ...
                          <tr>
                            <td colSpan={9} style={{ background: 'var(--paper-2)' }}>
                              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))', gap: 10 }}>
                                <KcbDetail label="KCB transaction id" value={n.kcbTransactionId} mono />
                                <KcbDetail label="Request id" value={n.requestId} mono />
                                <KcbDetail label="Customer reference" value={n.customerReference} mono />
                                <KcbDetail label="Till number" value={n.tillNumber} mono />
                                <KcbDetail label="Organisation short code" value={n.organizationShortCode} mono />
                                <KcbDetail label="Paid into" value={n.creditAccountIdentifier} mono />
                                <KcbDetail label="Transaction time" value={n.transactionAt ? kcbWhen(n.transactionAt) : null} />
                                <KcbDetail label="Bank ledger line" value={n.bankTransactionId === null ? null : `#${n.bankTransactionId}`} />
                                <KcbDetail label="Reconciled" value={n.matchedAt ? `${kcbWhen(n.matchedAt)}${n.matchedBy ? ` by user #${n.matchedBy}` : ''}` : 'Not reconciled'} />
                                <KcbDetail label="Narration" value={n.narration} />
                              </div>
                            </td>
                          </tr>
                        )}
          ...
          {!loading && (
            <Pager page={page} pageSize={pageSize} total={total} pageSizes={KCB_PAGE_SIZES}
      ...
      {tab === 'setup' && (
        <>
          <section className="card card-pad" style={{ marginBottom: 14 }}>
            <div className="card-head"><h3>Inbound URLs for KCB</h3></div>
            <p className="muted">
              Hand these to KCB exactly as they appear. KCB posts payment notifications to them and this server
              answers with an acknowledgement.
            </p>
            <div className="code-scroll">
              <pre>{KCB_INBOUND_PATHS.map(([path, label]) => `${label}\n${window.location.origin}${path}`).join('\n\n')}</pre>
            </div>
          </section>
          ...
          <section className="card card-pad" style={{ marginBottom: 14 }}>
            ...
            <p>
              ...
              raw request body, signed with KCB's private key and checked here against the public key stored on
              this integration.
            </p>
            <p className="muted">
              This is not OAuth. No token is exchanged and nothing is sent back to KCB, so that key is the only
          ...
          <section className="card card-pad" style={{ marginBottom: 14 }}>
            ...
            {config === null ? (
              ...
              <>
                <KcbSecretState label="KCB public key (verifies inbound notifications)" present={config.publicKeyPresent}
                  note={config.publicKeyFingerprint ? `Fingerprint ${config.publicKeyFingerprint}` : 'Stored, but it does not parse as a public key'} />
                <KcbSecretState label="Environment key" present={config.envKeyPresent}
                  note="KCB_PUBLIC_KEY on the server. Used when no key is stored here." />
                <KcbSecretState label="Our private key (outbound, unused until transfers are enabled)" present={config.privateKeyPresent} />
                <KcbSecretState label="Consumer key" present={config.consumerKeyPresent} />
                <KcbSecretState label="Consumer secret" present={config.consumerSecretPresent} />
              </>
            )}
      ...
      {unmatchTarget && (
        ...
        >
          <p>
            <b>{kcbRef(unmatchTarget)}</b> will be unlinked from invoice{' '}
...
function KcbDetail({ label, value, mono }: { label: string; value: string | null | undefined; mono?: boolean }) {
  return (
    <div>
 ...
 */
function KcbMatchDialog({ notification, currency, busy, onClose, onSave }: {
  notification: KcbNotification;
  currency: string | null;
  busy: boolean;
  onClose: () => void;
  ...
  const [candidates, setCandidates] = useState<KcbInvoiceCandidate[]>([]);
      ...
      () => {
        void (async () => {
          ...
          try {
            ...
            if (search) params.set('search', search);
            params.set('limit', '20');
            const r = await api<{ data: KcbInvoiceCandidate[] }>(`/api/ops/finance/kcb/invoices?${params.toString()}`);
  ...
  return (
    ...
    >
      <div className="record-card" style={{ marginBottom: 12 }}>
        <div className="record-card-top">
          <b className="cell-mono">{kcbRef(notification)}</b>
          <b>{kcbMoney(notification.amount, currency)}</b>
        </div>
        <div className="record-card-meta">
          {notification.customerName || 'Payer not named'}
          {' \u00B7 '}
          {kcbWhen(notification.transactionAt)}
        </div>
      </div>

      <div className="field">
        <label htmlFor="kcb-invoice-search">Find the invoice this payment settles</label>
        <input
          id="kcb-invoice-search"
          className="search-input"
          type="search"
            ...
            <tbody>
              {candidates.map((c) => (
                <tr key={c.id} className={selected === c.id ? 'row-click is-selected' : 'row-click'} onClick={() => setSelected(c.id)}>
                  ...
                  <td>{kcbDay(c.invoiceDate)}</td>
                  <td className="cell-num">{kcbMoney(c.outstanding, c.currency)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {chosen && (
        <p className="muted" style={{ marginTop: 12 }}>
          {difference === null
            ? `Invoice ${chosen.invoiceNo} is outstanding by ${kcbMoney(chosen.outstanding, chosen.currency)}.`
            : difference === 0
              ? `This payment settles invoice ${chosen.invoiceNo} exactly.`
              : difference > 0
                ? `This payment exceeds the outstanding balance on ${chosen.invoiceNo} by ${kcbMoney(difference, chosen.currency)}. An over-payment is allowed and recorded exactly as received.`
                : `This payment is short of the outstanding balance on ${chosen.invoiceNo} by ${kcbMoney(Math.abs(difference), chosen.currency)}. The invoice stays open for the remainder.`}
        </p>
      )}
 ...
 */
function KcbConfigDialog({ config, accounts, busy, error, onClose, onSave }: {
  config: KcbConfigView | null;
  accounts: KcbBankAccount[];
  busy: boolean;
  error: string;
  onClose: () => void;
  ...
  const secretField = (key: string, label: string, present: boolean, multiline: boolean, hint: string) => {
    ...
    return (
      <div className="field" key={key}>
        <label htmlFor={`kcb-secret-${key}`}>{label}</label>
        {multiline ? (
          <textarea
            id={`kcb-secret-${key}`}
            className="search-input"
        ...
        <div style={{ marginTop: 6 }}>
          <KcbSecretState label={`${label} status`} present={isClearing ? false : present} note={isClearing ? 'Will be removed when you save.' : hint} />
          {present && (
            <button
              type="button"
  ...
  return (
    <Modal
      title="KCB integration settings"
      wide
      onClose={() => { if (!busy) onClose(); }}
      footer={
        <>
    ...
    >
      <p className="muted">
        KCB signs each notification with its own private key. Paste KCB&apos;s <b>public</b> key here so the
      ...
      <div className="field">
        <label htmlFor="kcb-account-currency">Settlement account</label>
        <select
          id="kcb-account-currency"
          className="search-input"
          value={bankAccountId}
      ...
      <div className="field">
        <label htmlFor="kcb-environment">Environment</label>
        <select
          id="kcb-environment"
          className="search-input"
          value={environment}
      ...
      <div className="field">
        <label htmlFor="kcb-country">Country</label>
        <input
          id="kcb-country"
          className="search-input"
          placeholder="KE"
        ...
        />
        <span className="muted" style={{ fontSize: 12 }}>
          KCB operates in Kenya, Uganda, Tanzania, Rwanda, Burundi and South Sudan. Set the country this
          company collects in, not the country the bank is headquartered in.
        </span>
      </div>

      <div className="field">
        <label htmlFor="kcb-currency">Currency</label>
        <input
          id="kcb-currency"
          className="search-input"
          placeholder="KES"
      ...
      <div className="field">
        <label htmlFor="kcb-short-code">Organisation short code</label>
        <input
          id="kcb-short-code"
          className="search-input"
          placeholder="As issued by KCB"
      ...
      <div className="field">
        <label htmlFor="kcb-till">Till number</label>
        <input
          id="kcb-till"
          className="search-input"
          placeholder="As issued by KCB"
      ...
      <div className="field">
        <label htmlFor="kcb-gateway">Gateway base URL</label>
        <input
          id="kcb-gateway"
          className="search-input"
        ...
        />
        <span className="muted" style={{ fontSize: 12 }}>
          Used by Test connection. Change this only when KCB moves you between their UAT and production
          gateways.
        </span>
      </div>
      ...
      {secretField('publicKey', 'KCB public key', Boolean(config?.publicKeyPresent), true, config?.publicKeyFingerprint ? `Fingerprint ${config.publicKeyFingerprint}` : 'Required before any notification can be verified.')}
      ...
      {secretField('consumerKey', 'Consumer key', Boolean(config?.consumerKeyPresent), false, 'Not used by notifications; kept for the APIs KCB issues alongside them.')}
      {secretField('consumerSecret', 'Consumer secret', Boolean(config?.consumerSecretPresent), false, 'Not used by notifications; kept for the APIs KCB issues alongside them.')}
    </Modal>
  );
}