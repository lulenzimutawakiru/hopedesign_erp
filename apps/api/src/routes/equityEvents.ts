/**
 * Equity inbound notification receiver (unauthenticated bank webhook).
 *
 *   POST /api/integrations/equity/account-notification
 *   POST /api/integrations/equity/till-notification
 *   POST /api/integrations/equity/validation
 *
 * Equity is the caller, so the RSA signature over the RAW request body is the
 * authentication (services/equity/security.ts). The raw bytes are the signed
 ...
 * hands it to the ingest service, and renders Equity's acknowledgement contract.
 ...
 * failure makes Equity retry, and a retry cannot repair an invalid signature or an
...
import {
  ingestEquityNotification,
  readSignatureHeader,
  validateEquityBill,
  EquityBillValidationResult,
  EquityIngestResult,
} from '../services/equity/ingest.js';
import { isRecord, str, EquityNotificationType } from '../services/equity/payload.js';
import { ApiError, asyncHandler } from '../utils.js';

export const equityEventsRouter = Router();
...
/** Identifiers Equity sends that its acknowledgement contract echoes back. */
...
function prepare(req: Request, notificationType: EquityNotificationType) {
...
/**
 * Equity reads statusCode and nothing else: '0' is accepted, anything else is
...
function accountAcknowledgement(result: EquityIngestResult, ids: AckIdentifiers) {
  return {
...
function tillAcknowledgement(result: EquityIngestResult, ids: AckIdentifiers) {
  return {
    header: {
 ...
 */
function validationAcknowledgement(result: EquityBillValidationResult) {
...
equityEventsRouter.post(
  '/account-notification',
  notificationLimiter,
  rawBodyReader,
  asyncHandler(async (req, res) => {
    const { options, ack } = prepare(req, 'ACCOUNT');
    const result = await ingestEquityNotification(options);
...
equityEventsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await validateEquityBill(options);
 ...
 */
export function equityWebhookErrorFilter(err: unknown, req: Request, res: Response, next: NextFunction): void {
  ...
  console.error(`[equity][webhook] ${req.method} ${req.originalUrl}`, err);
