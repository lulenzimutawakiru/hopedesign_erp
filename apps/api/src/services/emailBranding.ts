...
export function renderBrandedEmailHtml(opts: BrandedEmailOptions): string {
  ...
  for (let i = 0; i < contactPairs.length; i += 2) {
    ...
    const cells = pair.map(([label, value, href]) => {
      ...
      return '<td width="50%" valign="top" style="padding:0 16px 14px 0;font-family:' + FONT + ';">'
        + '<div style="margin:0 0 3px;font-size:11px;line-height:1.3;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;color:' + quiet + ';">' + escapeHtml(String(label)) + '</div>'
  ...
  const marks: Record<string, string> = { Facebook: 'f', Instagram: 'Ig', LinkedIn: 'in', X: 'X', YouTube: 'Yt', TikTok: 'Tt', WhatsApp: 'Wa' };
...
function emailIdentityText(company: CompanyBrand | undefined) {
