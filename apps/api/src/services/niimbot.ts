...
/** Resolve the label variety (template) to use for a QR of a given kind. */
async function resolveLabelTemplate(
  ...
  if (templateId != null && Number.isFinite(Number(templateId))) {
    const res = await client.query(
      `SELECT id, mm_width, mm_height, printer_model FROM label_templates
       WHERE id = $1 AND tenant_id = $2 AND kind = $3`,
      [Number(templateId), tenantId, kind]
    );
    if (res.rows.length === 0) {
      throw badRequest(`Label template ${templateId} not found for ${kind} labels`);
    }
    const row = res.rows[0];
    return {
      id: Number(row.id),
      mmWidth: row.mm_width != null ? Number(row.mm_width) : null,
      mmHeight: row.mm_height != null ? Number(row.mm_height) : null,
      printerModel: row.printer_model != null ? String(row.printer_model) : null,
    };
  }
  // No explicit variety - fall back to the tenant default (then first active) for the kind.
  const res = await client.query(
    `SELECT id, mm_width, mm_height, printer_model FROM label_templates
     WHERE tenant_id = $1 AND kind = $2 AND is_active = true
...
async function labelContent(
  ...
  mm?: { width: number; height: number } | null
): Promise<{ title: string; lines: string[]; mm: { width: number; height: number } }> {
  const size = mm && mm.width > 0 && mm.height > 0
    ? mm
    : LABEL_MM[String(qr.entity_type)] ?? LABEL_MM.REAM;
  const base = [
  ...
  return { title: String(qr.entity_type ?? 'LABEL'), lines: base, mm: size };
}
...
export async function spoolLabels(
  ...
  const items = [...new Map<number, SpoolItem>((input.items ?? []).map((it) => [Number(it.qrId), it] as [number, SpoolItem])).values()].filter(
  ...
  const templateCache = new Map<string, Awaited<ReturnType<typeof resolveLabelTemplate>>>();
  const templateFor = async (kind: string): Promise<Awaited<ReturnType<typeof resolveLabelTemplate>>> => {
    if (!templateCache.has(kind)) {
      templateCache.set(kind, await resolveLabelTemplate(client, tenantId, kind, input.templateId));
    }
    return templateCache.get(kind) ?? null;
  };
  ...
  for (const item of items) {
    ...
    const kind = String(qr.entity_type).toUpperCase();
    const template = await templateFor(kind);
    const mm = template?.mmWidth && template?.mmHeight
      ? { width: template.mmWidth, height: template.mmHeight }
      : null;
    ...
    const content = await labelContent(client, qr, mm);
    ...
    const ins = await client.query(
      ...
      [companyId, tenantId, qr.id, template?.id ?? input.templateId ?? null, labelNo, jobId, relPath]
    );
    labels.push({
