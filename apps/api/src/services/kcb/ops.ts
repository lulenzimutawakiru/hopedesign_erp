/**
 * KCB operations surface: status, configuration and notification reconciliation
 * for the finance workspace.
 *
 * Everything in this module is a human, permissioned action. The inbound
 * receiver (services/kcb/ingest.ts) is the only code that accepts money; this is
 ...
 *
 * The self-test is deliberately local. KCB publishes no unauthenticated health
...
export interface KcbNotificationView {
  id: number;
...
export function kcbNotificationView(row: Row): KcbNotificationView {
  return {
    id: Number(row.id),
    notificationType: String(row.notification_type ?? ''),
    kcbTransactionId: text(row.kcb_transaction_id),
...
const NOTIFICATION_SELECT = `
  n.id, n.notification_type, n.kcb_transaction_id, n.request_id, n.transaction_reference,
...
const NOTIFICATION_FROM = `
  FROM kcb_payment_notifications n
  LEFT JOIN bank_accounts ba ON ba.id = n.bank_account_id
  LEFT JOIN customer_invoices ci ON ci.id = n.matched_invoice_id`;

const readNotification = async (client: pg.PoolClient, companyId: number, id: number): Promise<KcbNotificationView> => {
  ...
  return kcbNotificationView(res.rows[0] as Row);
};

/** The settlement account an integration is pointed at, with its currency. */
export interface KcbSettlementAccount {
  id: number;
  code: string;
  name: string;
  currency: string;
...
async function settlementAccount(
  ...
  companyId: number,
  id: number | null
): Promise<KcbSettlementAccount | null> {
 ...
 */
function configWarnings(config: KcbConfigView, account: KcbSettlementAccount | null): string[] {
  const warnings: string[] = [];
  if (config.publicKeyFingerprint === null && !config.envKeyPresent) {
    warnings.push('No usable KCB public key is stored, so every inbound notification will be refused.');
  } else if (config.publicKeyPresent && config.publicKeyFingerprint === null) {
    warnings.push('The stored KCB public key could not be read as an RSA public key. Paste the full PEM block.');
  }
...
export async function kcbConfig(client: pg.PoolClient, ctx: Ctx): Promise<KcbConfigView> {
  return kcbConfigView(await getKcbIntegration(client, ctx));
}

export interface KcbConfigSaveResult {
  config: KcbConfigView;
  warnings: string[];
}

/** Coerce an administrator's payload into the config patch the service applies. */
export function kcbConfigPatchFrom(body: Record<string, unknown>): KcbConfigPatch {
...
export async function updateKcbFromPatch(
  client: pg.PoolClient,
  ctx: Ctx,
  body: Record<string, unknown>
): Promise<KcbConfigSaveResult> {
  const config = await updateKcbConfig(client, ctx, kcbConfigPatchFrom(body));
...
export interface KcbConnectionCheck {
  key: string;
  label: string;
  ok: boolean;
  detail: string;
...
export interface KcbConnectionTestResult {
  ok: boolean;
 ...
 */
export async function testKcbConnection(client: pg.PoolClient, ctx: Ctx): Promise<KcbConnectionTestResult> {
  const companyId = requireCompany(ctx);
  const row = await getKcbIntegration(client, ctx);
  const config = kcbConfigView(row);
  const stored = readKcbConfig(row);
  const account = await settlementAccount(client, companyId, config.bankAccountId);

  const checks: KcbConnectionCheck[] = [
    {
      key: 'integration',
      label: 'Integration record',
      ok: config.configured,
      detail: config.configured ? `Configured for company ${companyId}` : 'No KCB integration exists for this company yet',
      critical: true,
    },
    {
      key: 'signing_key',
      ...
      detail: config.publicKeyFingerprint !== null
        ...
        : config.envKeyPresent
          ? 'Loaded from the platform KCB_IPN_PUBLIC_KEY environment variable'
          : config.publicKeyPresent
            ? 'A key is stored but it does not parse as an RSA public key'
            : 'No public key stored; KCB notifications cannot be verified',
      critical: true,
    },
    {
  ...
  await markKcbTested(client, ctx, status);
  await auditConfig(
    client,
    ctx,
    'test',
    ...
    null,
    { ok, checks },
    { code: KCB_CODE, environment: config.environment }
  );
  return { ok, status, testedAt: new Date().toISOString(), checks, config: await kcbConfig(client, ctx) };
}

export interface KcbStatusTotals {
  total: number;
  received: number;
  posted: number;
  matched: number;
...
export interface KcbStatusView {
  config: KcbConfigView;
...
export async function kcbStatus(client: pg.PoolClient, ctx: Ctx): Promise<KcbStatusView> {
  const companyId = requireCompany(ctx);
  const row = await getKcbIntegration(client, ctx);
  const config = kcbConfigView(row);
  ...
  const grouped = await client.query<{ status: string; is_validation: boolean; count: string | number; amount: string | number | null }>(
    `SELECT status,
       ...
       FROM kcb_payment_notifications
      WHERE company_id = $1
      GROUP BY status, is_validation`,
    [companyId]
  );

  const totals: KcbStatusTotals = {
    total: 0,
    received: 0,
    posted: 0,
    matched: 0,
  ...
  const last = await client.query('SELECT max(created_at) AS last_at FROM kcb_payment_notifications WHERE company_id = $1', [companyId]);
  const account = await settlementAccount(client, companyId, config.bankAccountId);
  return {
    config,
    totals,
...
export interface KcbNotificationFilters {
  status?: string;
  notificationType?: string;
  bankAccountId?: number | null;
...
export interface KcbNotificationPage {
  rows: KcbNotificationView[];
  total: number;
  limit: number;
  offset: number;
}

export async function listKcbNotifications(
  client: pg.PoolClient,
  ctx: Ctx,
  filters: KcbNotificationFilters
): Promise<KcbNotificationPage> {
  const companyId = requireCompany(ctx);
  ...
  if (filters.search) {
    add(
      (i) =>
        `(n.kcb_transaction_id ILIKE '%' || $${i} || '%' OR n.transaction_reference ILIKE '%' || $${i} || '%'` +
  ...
  const total = await client.query<{ count: string | number }>(
    `SELECT count(*)::int AS count FROM kcb_payment_notifications n WHERE ${clause}`,
    params
  );
  return {
    rows: rows.rows.map((r) => kcbNotificationView(r as Row)),
...
export interface KcbMatchResult {
  notification: KcbNotificationView;
 ...
 */
export async function matchKcbNotification(
  client: pg.PoolClient,
  ctx: Ctx,
  id: number,
  invoiceId: number
): Promise<KcbMatchResult> {
  const companyId = requireCompany(ctx);
  ...
  await auditConfig(
    ...
    { code: KCB_CODE, amount: existing.amount, currency: existing.currency, difference }
  );
...
export async function unmatchKcbNotification(
  client: pg.PoolClient,
  ctx: Ctx,
  id: number
): Promise<KcbNotificationView> {
  const companyId = requireCompany(ctx);
  const existing = await readNotification(client, companyId, id);
...
export interface KcbInvoiceCandidate {
  id: number;
  invoiceNo: string;
  customerName: string | null;
  currency: string | null;
 ...
 */
export async function searchKcbInvoices(
  client: pg.PoolClient,
  ctx: Ctx,
  opts: { search?: string; limit?: number | null } = {}
): Promise<KcbInvoiceCandidate[]> {
  const companyId = requireCompany(ctx);
