/**
 * Normalisation of the two KCB notification envelopes into one record.
 *
 ...
 *
 * KCB's field names differ between the two (transactionAmount vs transactionAmt,
 ...
 */
export type KcbNotificationType = 'ACCOUNT' | 'TILL' | 'VALIDATION';

/** The canonical, storage-shaped view of one inbound notification. */
export interface KcbNotification {
  ...
  balance: number | null;
  /** ISO-8601 instant, or null when KCB sent something we cannot read. */
...
/**
 * UTC offsets in minutes for the zone tokens KCB puts in textual dates. KCB is a
 * Kenyan group bank and stamps the till leg in EAT; an unrecognised token falls
 * back to the caller's default rather than silently assuming UTC and shifting
 * the transaction onto the wrong day.
 */
const ZONE_OFFSETS: Record<string, number> = {
  EAT: 180, CAT: 120, WAT: 60, SAST: 120, GMT: 0, UTC: 0,
};

/** Default zone offset (EAT, KCB's home market). */
...
/** Case-insensitive field read: KCB mixes timeStamp / timestamp / transactionID. */
...
/**
 * Parse the date KCB sends, in any of the three shapes it actually uses:
 ...
 */
export function parseKcbTransactionDate(
  value: unknown,
...
export function normalizeKcbNotification(
  body: unknown,
  opts: { defaultOffsetMinutes?: number; notificationType?: KcbNotificationType } = {}
): KcbNotification | null {
  if (!isRecord(body)) return null;
  const bag = flatten(body);
  const notificationType: KcbNotificationType =
  ...
  return {
    ...
    // the only thing that can stop a KCB retry becoming a second ledger line.
    kcbTransactionId: first('transactionID', 'transactionId', 'transactionReference'),
    ...
    transactionAt: parseKcbTransactionDate(
