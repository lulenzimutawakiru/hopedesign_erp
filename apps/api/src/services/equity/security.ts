/**
 * Equity inbound notification security primitives.
 *
 * The notification endpoints are unauthenticated by nature - Equity is the caller
 * - so the RSA signature IS the authentication. Equity signs the raw request body
 * with its private key (SHA256withRSA) and puts the base64 signature in the
 * `Signature` header; we verify it with Equity's public key.
 *
...
/**
 * Verify the `Signature` header against the exact bytes Equity sent.
 *
