...
import { formatDocDateTime, loadCompanyProfile } from './branding.js';
...
export async function exportAssets(client: pg.PoolClient, ctx: Ctx, b: Record<string, unknown>) {
  ...
  let issuedBy = 'System';
  if (ctx.userId != null) {
    const u = await client.query('SELECT first_name, last_name FROM users WHERE id = $1', [ctx.userId]);
    const name = [u.rows[0]?.first_name, u.rows[0]?.last_name].filter(Boolean).join(' ').trim();
    if (name) issuedBy = name;
  }
  const reportType = s(b.reportType) ?? 'Asset Register';
  const data = {
    code: `ASSET-REGISTER-${issuedAt.slice(0, 10)}`,
    title: reportType,
    kicker: 'Asset register',
    subtitle: `${rows.length} asset${rows.length === 1 ? '' : 's'}`,
    currency: 'UGX',
    facts: [
      ['Assets', String(rows.length)],
      ['Generated', formatDocDateTime(issuedAt)],
      ['Prepared by', issuedBy],
